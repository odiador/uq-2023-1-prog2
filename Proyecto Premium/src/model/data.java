...
public class data implements Serializable {
	...
	public data() {
		 ...
		 * cine están o no ocupadas - En la 1 está todo lo relacionado con el cliente
		 * (ArrayList<Object>): ■ 0: Nombre (String) ■ 1: Cedula (String) ■ 2: Veces que
		 * ha ido a cine en el año (lo del año no está hecho) (int) ■ 3: Puntos (int) ■
		 * 4: Si tiene tarjeta basic (boolean) ■ 5: Si tiene tarjeta gold (boolean) ■ 6:
		 * El dinero en la tarjeta (double) - En la 2 está la información de compra de
		 * cada lugar de cine en una matriz (ArrayList<Object>): Si la silla esta
		 * desocupada el sitio está vacío, si no: ■ 0: Hora (int) ■ 1: Minutos (int) ■
		 * 2: Segundos (int) ■ 3: Día (int) ■ 4: Numero de mes (int) ■ 5: Año (int) ■ 6:
		 * Tipo(s) de pago (ArrayList<String>) ■ 7: Nombre (String) ■ 8: Cedula (String)
		 * ■ 9: Veces que ha ido a cine en el año (en ese momento) (int) ■ 10: Puntos
		 * (en ese momento) (int) ■ 11: Si tiene tarjeta basic (en ese momento)
		 * (boolean) ■ 12: Si tiene tarjeta gold (en ese momento) (boolean) ■ 13: El
		 * dinero en la tarjeta (en ese momento) (double) - En la 3 hay datos
		 * importantes para todas las clases (ArrayList<Object>), a futuro se pueden
		 * usar mas cosas: ■ 0: Año "lectivo" (int)
		 */
		try {
			...
			try {
				...
				ObjectOutputStream carpetaEscribir = new ObjectOutputStream(
						new FileOutputStream("src/model/carpeta.dat"));
				base.add(cine);
				base.add(clientes);
	...
	/**
	 * Sobreescribe un dato de la lista de un cliente
	 *
	 * @param objeto El objeto a reemplazar
	 * @param i      La posición en la que
	 */
	...
	/**
	 * Guarda la informacion en un archivo llamado <b>carpeta.dat</b>
	 */
	...
	/**
	 * Busca la posición en la que se encuentra un cliente. <br>
	 * Si no lo encuentra se retorna un -7
	 *
	 * @param cedula La cédula de la persona a buscar
	 * @return La posición en la lista de la persona
	 */
