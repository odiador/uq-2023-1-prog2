...
import objetos.BotonListener;
import objetos.Confiteria;
import objetos.CustomBoton;
import objetos.CustomFormatTextfield;
import objetos.ListadeConfiteria;

public class VentanaPagoConfit extends ParteSuperior implements BotonListener {
	...
	private CustomFormatTextfield tf, tf2;
	private double total;
	private CustomBoton combo1, combo2, comboTipoPago, pagar, volver;
	...
	public VentanaPagoConfit(ListadeConfiteria listaConfiteria) {
		...
		comboTipoPago = new CustomBoton(opciones);
		comboTipoPago.setFont(Herramientas.FUENTE_COOLVETICA);
		comboTipoPago.addBotonListener(this);
		...
		tf = new CustomFormatTextfield(Herramientas.setFormatodeNumero(0, getTotal()));
		tf.setText(Herramientas.formatoSinDecimal(getTotal()));
		tf.setFont(Herramientas.FUENTE_COOLVETICA.deriveFont(25f));
		tf.setEditable(false);
		tf.addKeyListener(this);

		tf2 = new CustomFormatTextfield(Herramientas.setFormatodeNumero(0, getTotal()));
		tf2.setText("0");
	...
	public void BotonPresionado(CustomBoton c) {
		if (c.equals(volver)) {
		}
		if (c.equals(pagar)) {
