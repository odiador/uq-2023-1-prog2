...
public abstract class ParteSuperior extends JFrame implements MouseListener, MouseMotionListener, KeyListener {
	...
	private boolean titleAffectsColor = false;
	...
	public ParteSuperior() {
		...
		setFont(Herramientas.FUENTE_TITULO_DEFAULT);
	...
	public void mouseEntered(MouseEvent e) {
		...
		if (e.getSource() == stellarCinema) {
			...
			setYaSeCambiodeColor(true);
			hiloColor.start();
		}
		if (isTitleAffectsColor()) {
	...
	public void setRedimensionable(boolean opcion) {
		...
		if (!opcion) {
			...
			panelArrastre.setSize(getBounds().width - 102, 30);
		} else {
	...
	public void setStellarVisible(boolean res) {
		...
		setTitleAffectsColor(!res);
		if (!res) {
			panelSuperior.setSize(panelSuperior.getWidth(), separadorSup.getY() + separadorSup.getHeight());

		} else {
			panelSuperior.setSize(panelSuperior.getWidth(), stellarCinema.getY() + stellarCinema.getHeight());
		}
	}
	...
	public static boolean isSeCambiodeColor() {
		return yaSeCambiodeColor;
	}

	public static void setYaSeCambiodeColor(boolean yaSeCambiodeColor) {
		ParteSuperior.yaSeCambiodeColor = yaSeCambiodeColor;
	}

	public boolean isTitleAffectsColor() {
		return titleAffectsColor;
	}

	public void setTitleAffectsColor(boolean titleAffectsColor) {
		this.titleAffectsColor = titleAffectsColor;
	}
